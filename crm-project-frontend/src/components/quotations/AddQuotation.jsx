...
export default function AddQuotation({ id = null, onBack, leadData = null }) {
        ...
        if (Array.isArray(d.items) && d.items.length > 0) {
          setItems(
            d.items.map((it, idx) => ({
              ...
              cars: it.cars !== undefined && it.cars !== null ? String(it.cars) : "",
              userEditedCars: it.cars !== undefined && it.cars !== null,
            }))
          );
  ...
  const handleItemChange = (index, field, value) => {
    setItems((prev) => {
      ...
      if (field === "cars") {
        currentItem.userEditedCars = true;
      }
      ...
      if (field === "parking_product_id") {
        ...
        if (prod) {
          ...
          if (!currentItem.userEditedCars) {
            const defaultCars = (parseInt(prod.car_capacity) || 1) * (parseInt(currentItem.quantity) || 1);
            currentItem.cars = String(defaultCars);
          }
      ...
      if (field === "quantity" && !currentItem.userEditedCars) {
        const prod = products.find((p) => String(p.id) === String(currentItem.parking_product_id));
        if (prod) {
          const defaultCars = (parseInt(prod.car_capacity) || 1) * (parseInt(value) || 1);
          currentItem.cars = String(defaultCars);
        }
      }
  ...
  const summary = useMemo(() => {
    ...
    const computedItems = items.map((it) => {
      ...
      const defaultCap = prod ? (parseInt(prod.car_capacity) || 1) * q : q;
      const itemCars = (it.cars !== undefined && it.cars !== null && it.cars !== "") ? (parseInt(it.cars) || 0) : defaultCap;

      totalCarsCount += itemCars;
  ...
  const handleSubmit = async (e) => {
    ...
    const itemsPayload = items.map((it, idx) => {
      const comp = summary.computedItems[idx];
      return {
        ...
        description: it.description || "",
        cars: comp ? comp.cars : (parseInt(it.cars) || 1)
      };
    });
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          min="1"
                          value={item.cars ?? ""}
                          onChange={(e) => handleItemChange(index, "cars", e.target.value)}
                          placeholder="e.g. 10"
                          className="w-full px-3 py-2 rounded-md border border-slate-200 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition bg-white"
                        />
                      </div>
